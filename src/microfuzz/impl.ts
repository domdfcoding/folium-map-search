...
function scoreConsecutiveLetters(
	...
	indices.forEach(([firstIdx, lastIdx]) => {
		...
		const isStartOfWord = firstIdx === 0 || normalizedItem[firstIdx] === ' '
			|| normalizedItem[firstIdx - 1] === ' ';
		const isEndOfWord = lastIdx === normalizedItem.length - 1
...
export function fuzzyMatchImpl(text: string, query: string): FuzzyResult<string> | null {
